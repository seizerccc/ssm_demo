...
<body>
	...
	<div class="page-container">
		<h1>登录</h1>
		<p>path:<%=path %></p>
		<p>bathPath：<%=basePath%></p>
		<form action="/ssm_maven/login" method="post">
			<input type="text" id="username" name="username" class="username"
				placeholder="请输入用户名（6-15位字母数字）" pattern="[\w\s]{6,15}" required="required">
			<input type="password" id="password" name="password" class="password"
				pattern="[\w\s]{6,12}" required="required" placeholder="请输入密码（6-12位字母数字）">
